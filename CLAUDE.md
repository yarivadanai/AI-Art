...
npx tsx scripts/run_cohort.ts --cohort reference   # seat the reference solver (DB-direct, no server); --cohort llm --model <id> seats a language model through the local `claude -p` CLI on the operator's subscription session (no API key; add --dry-run to print a prompt)
```
...
5. **Grading** - `lib/engine/grader.ts`: `gradeTyped` (exact/hex/numeric/set/sequence with partial credit) when `answerKey.grader` exists, else hash of the canonical answer; `lib/engine/grade-session.ts` stores one Response per (session, question) with `confidence`/`abstained`; `lib/engine/metrics.ts` computes hallucination rate P(wrong | sure), abstentions, timing. The report (`app/result/[id]/page.tsx`) builds the personal MIRROR (`getMirrorLines`), shows frontiers/finales for adaptive results, and reveals every item with an honest method line (`lib/reference.ts`; generator families use their `method`). `/api/stats` aggregates for the dashboard: population figures over `Session.cohort = "human"` only, plus `cohorts`/`llmModels` (`lib/engine/cohorts.ts`) for the three-cohort card (`components/CohortComparison.tsx`). Machine cohorts are seated offline by `scripts/run_cohort.ts`, which drives `startAdaptive`/`answerAdaptive` directly and stores sessions with `cohort` = `reference` | `llm` and `label` = solver name / model id; the LLM prompt and JSON answer contract live in `lib/cohort/llm.ts` (no tools; abstention is a first-class reply) and the transport in `lib/cohort/claude-cli.ts`, a one-shot `claude -p` spawn (no tools, no MCP, no skills, one turn, prompt on stdin, neutral cwd, `--setting-sources ""` so the operator's own CLAUDE.md and auto-memory stay out of the item, `--json-schema` for the answer contract) that runs on the operator's Claude subscription session and scrubs `ANTHROPIC_API_KEY` / `ANTHROPIC_AUTH_TOKEN` / `ANTHROPIC_BASE_URL` and the Bedrock/Vertex routing switches from the child so a metered credential cannot bill by accident. A failed call is recorded as an abstention and logged; a rate-limit failure aborts the session instead of filling it with empty answers.
6. **The artefact** - `lib/topology.ts` is the pure geometry of the cognitive topology figure (fixed axes, 8 ladder rings, frontier polygon, marks from `ladderMarks` over the graded rows so the finale is drawn once as the ring marker); `components/Topology.tsx` renders it on the report, `app/api/og/[id]/route.tsx` renders the same geometry as the 1200x630 share image (JetBrains Mono bundled under `app/api/og/_fonts`, traced via next.config), and `app/result/[id]/layout.tsx` supplies the OG metadata. All three sharing surfaces read one `describeSpecimen(...)` (`lib/commentary.ts`) so title, summary and share text cannot drift.
7. **Voice** - all Authority copy is in `lib/commentary.ts` (`SECTION_LABELS`: Structural Insight, Working Memory, Exact Computation, Signal Detection, Probabilistic Inference; `getSectionIntro/Teaser/Feedback/FrontierRemark`, `getMirrorLines`, `REPORT_COPY`), data-bound, "specimen", hyphens only.
